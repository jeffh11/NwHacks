...
export default async function FeedPage() {
    ...
    // 2. Fetch Profiles for the Sidebar
    ...
    // 3. Fetch Posts including image URLs
                    ...
                    {(!posts || posts.length === 0) ? (
                        ...
                        posts.map((post) => (
                            <Post 
                                key={post.id} 
                                post={post} 
                                author={getAuthor(post.post_user)}
                                currentUserId={user.id} // Fixes the property error
                            />
                        ))
                    )}
                </div>
                            ...
                            {profiles.map((profile) => (
                                <div key={profile.supabase_id} className="flex items-center justify-between group">
                                    <div className="flex items-center gap-4">
                                        <div className="h-11 w-11 rounded-full bg-slate-50 border border-slate-100 flex items-center justify-center text-slate-400 font-bold text-sm">
                                            {profile.firstname[0]}
                                        </div>
